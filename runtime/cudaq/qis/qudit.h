...
public:
  ...
  qudit() : idx(get_execution_manager().allocateQudit(n_levels())) {}
  qudit(const std::vector<complex> &state) : qudit() {
    ...
    get_execution_manager().initializeState({QuditInfo(n_levels(), idx)},
  ...
  ~qudit() { get_execution_manager().returnQudit({n_levels(), idx}); }
};
