...
void oneQubitApply(QubitArgs &...args) {
  ...
  if constexpr (std::is_same_v<mod, base>) {
    for (auto &qubit : quditInfos)
      get_execution_manager().apply(gateName, {}, {}, {qubit});
  ...
  if (!controls.empty())
    for (std::size_t i = 0; i < controls.size(); i++)
      if (qubitIsNegated[i])
        get_execution_manager().apply("x", {}, {}, {controls[i]});

  // Apply the gate
  get_execution_manager().apply(gateName, {}, controls, {quditInfos.back()},
...
void oneQubitApplyControlledRange(QubitRange &ctrls, qubit &target) {
  ...
  // Apply the gate
  get_execution_manager().apply(gateName, {}, controls,
...
void oneQubitSingleParameterApply(ScalarAngle angle, QubitArgs &...args) {
  ...
  if constexpr (nArgs > 1 && std::is_same_v<mod, base>) {
    for (auto &targetId : targets)
      get_execution_manager().apply(gateName, {angle}, {}, {targetId});
  ...
  // Apply the gate
  get_execution_manager().apply(gateName, {angle}, controls, {targets.back()},
                                std::is_same_v<mod, adj>);
}
...
void oneQubitSingleParameterControlledRange(ScalarAngle angle,
  ...
  // Apply the gate
  get_execution_manager().apply(gateName, {angle}, controls,
...
void oneQubitSingleParameterApply(ScalarAngle angle, QubitArgs &...args) {
  ...
  // We just want to apply the same gate to all qubits provided
  for (auto &targetId : targets)
    get_execution_manager().apply(gateName, std::vector<double>{angle}, {},
                                  {targetId});
}
...
void u3(ScalarAngle theta, ScalarAngle phi, ScalarAngle lambda,
  ...
  if constexpr (nArgs > 1 && std::is_same_v<mod, base>) {
    for (auto &targetId : targets)
      get_execution_manager().apply("u3", parameters, {}, {targetId});
    return;
  }
  ...
  // Apply the gate
  get_execution_manager().apply("u3", parameters, controls, {targets.back()},
                                std::is_same_v<mod, adj>);
}
template <typename mod = ctrl, typename ScalarAngle, typename QubitRange>
  requires(std::ranges::range<QubitRange>)
void u3(ScalarAngle theta, ScalarAngle phi, ScalarAngle lambda,
  ...
  // Apply the gate
  get_execution_manager().apply("u3", parameters, controls,
                                {qubitToQuditInfo(target)});
}
...
void cu3(ScalarAngle theta, ScalarAngle phi, ScalarAngle lambda, qubit &ctrl,
  ...
  get_execution_manager().apply("u3", parameters, controls, targets);
}
...
void swap(QubitArgs &...args) {
  ...
  if constexpr (nArgs == 2) {
    get_execution_manager().apply("swap", {}, {}, qubitIds);
    return;
  } else {
  ...
  get_execution_manager().apply("swap", {}, controls, targets);
}
...
void exp_pauli(double theta, QubitRange &&qubits, const char *pauliWord) {
  ...
  std::transform(qubits.begin(), qubits.end(), std::back_inserter(quditInfos),
                 [](auto &q) { return cudaq::qubitToQuditInfo(q); });
  get_execution_manager().apply("exp_pauli", {theta}, {}, quditInfos, false,
...
inline void reset(qubit &q) {
  get_execution_manager().reset({q.n_levels(), q.id()});
}
...
inline SpinMeasureResult measure(cudaq::spin_op &term) {
  return get_execution_manager().measure(term);
}
...
void control(QuantumKernel &&kernel, qubit &control, Args &&...args) {
  ...
  get_execution_manager().startCtrlRegion(ctrls);
  kernel(std::forward<Args>(args)...);
  get_execution_manager().endCtrlRegion(ctrls.size());
}
...
void adjoint(QuantumKernel &&kernel, Args &&...args) {
  ...
  get_execution_manager().startAdjointRegion();
  kernel(std::forward<Args>(args)...);
  get_execution_manager().endAdjointRegion();
}
...
void applyQuantumOperation(const std::string &gateName,
  ...
  if ((std::is_same_v<mod, base> || std::is_same_v<mod, adj>)&&NumT == 1) {
    for (auto &qubit : qubits)
      get_execution_manager().apply(gateName, parameters, {}, {qubit},
                                    std::is_same_v<mod, adj>);
    return;
  }
  ...
  // Apply the gate
  get_execution_manager().apply(gateName, parameters, controls, targets,
                                std::is_same_v<mod, adj>);
